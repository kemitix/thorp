...
class ParseConfigLinesTest extends FunSpec {
  ...
  describe("parse single lines") {
    ...
    describe("parallel") {
      describe("when valid") {
        it("should parse") {
          val expected =
            ConfigOptions.create(
              List[ConfigOption](ConfigOption.parallel(3)).asJava)
      ...
      describe("when invalid") {
        it("should ignore") {
          val expected =
            ConfigOptions.create(List.empty.asJava)
    ...
    describe("debug - true") {
      it("should parse") {
        val expected =
          ConfigOptions.create(List[ConfigOption](ConfigOption.debug()).asJava)
